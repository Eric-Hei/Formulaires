...
export function FormShortTextRenderer({ block, editor }: { block: any; editor: any }) {
  const { mode, answers, setAnswer, invalidFields } = useFormContext();
      ...
      if (mode === "edit") {
        return (
          <div className="form-block-wrapper group">
            ...
            <input
              ...
              disabled
              placeholder={block.props.placeholder || "Votre réponse"}
            />
            <input
              className="mt-1 w-full text-xs text-gray-400 bg-transparent border-none focus:outline-none"
              value={block.props.helpText}
              onChange={(e) =>
                editor.updateBlock(block, { props: { helpText: e.target.value } })
              }
...
export const FormShortText = createReactBlockSpec(
  {
    type: "formShortText" as const,
    propSchema: {
      label: { default: "Question" },
      placeholder: { default: "Votre réponse" },

...
export function FormMultipleChoiceRenderer({ block, editor }: { block: any; editor: any }) {
  const { mode, answers, setAnswer, invalidFields } = useFormContext();
      ...
      const selected = (answers[blockId] as string) ?? "";
            ...
            {options.map((opt, i) => (
              ...
              >
                <input
                  ...
                  onChange={() => setAnswer(blockId, opt)}
...
export const FormMultipleChoice = createReactBlockSpec(
  {
    type: "formMultipleChoice" as const,
    propSchema: {
  ...
  { render: FormMultipleChoiceRenderer }
);
